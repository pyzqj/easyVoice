...
import { BaseStorage } from '../storage/baseStorage.ts';
...
class CacheService {
  private storage: typeof BaseStorage;
  ...
  async get<T>(str: string): Promise<T | null> {
    ...
    return item.original === str ? item.value : null;
  }
  ...
  async has(str: string): Promise<boolean> {
    ...
    return !!(item && item.expireAt >= Date.now() && item.original === str);
  }
